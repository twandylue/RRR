...
mod tests {
    ...
    #[tokio::test]
    async fn sliding_window_redis_case1() -> Result<(), ()> {
        // prev
        ...
        for c in 0..11 {
            client
                .record_sliding_window(key_prefix, resource, subject, size)
                .await?;

            let count = client
                .fetch_sliding_window(key_prefix, resource, subject, size)
                .await?;
        ...
        let actual = client
            .can_make_request_sliding_window(key_prefix, resource, subject, size)
            .await?;
    ...
    #[tokio::test]
    async fn sliding_window_redis_case2() -> Result<(), ()> {
        // prev
        initialize_redis().await?;
